...
#undef MY_LOG
#endif
#define MY_LOG(args...) __android_log_print(ANDROID_LOG_INFO, "nsDOMSurfaceControl", ##args)
  ...
  , mProducer(NULL)
{
  ...
  //if (NS_FAILED(rv)) {
   // mListener->OnUserError(DOMSurfaceControlListener::kInStartCamera, rv);
  //}
}
...
nsDOMSurfaceControl::TrackCreated(TrackID aTrackID) {
  ...
  MediaInputPort* inputPort = nullptr;MY_LOG("%s:%d : this=%p", __func__, __LINE__, this);
  dom::VideoStreamTrack* track =
    new dom::VideoStreamTrack(this, aTrackID, aTrackID,
                              new BasicUnstoppableTrackSource(principal));MY_LOG("%s:%d : this=%p", __func__, __LINE__, this);
